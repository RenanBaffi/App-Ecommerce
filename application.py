...
def login():
    ...
    return jsonify({"message": "Unauthorized. Invalid credntials"}), 401
...
def remove_from_cart(product_id):
    ...
    return jsonify({'message': 'Failed to remove item form the cart'}), 400
...
def checkout():
    ...
    for cart_item in cart_items:
        db.session.delete(cart_item)
    db.session.commit()
    return jsonify ({'mesage': 'Checkout successfully. Cart has been cleared'})
